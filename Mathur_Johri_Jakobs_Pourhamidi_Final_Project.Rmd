...
set.seed(123)
inds <- sample(1:nrow(subCODGames), floor(.8*nrow(subCODGames)))
Train <- subCODGames[inds, ]
Test <- subCODGames [-inds, ]
set.seed(NULL)
```
...
```{r}
xvars <- unlist(names(select(subCODGames, -PrimaryWeapon)))
#"Map1","Map2","Choice","MapVote","Date","FullPartial","Result"      
#,"XPType","DidPlayerVote","GameType" 
subCODGames[, xvars] <- scale(subCODGames[,xvars], center = TRUE, scale = TRUE) 
...
for (x in 1:50) {
  knn_res <- knn(train = Train[,xvars,drop=FALSE], 
               ...
               k=x)
  rmsevec[x] <- sqrt((Test$PrimaryWeapon - knn_res$pred)^2)
}
```
